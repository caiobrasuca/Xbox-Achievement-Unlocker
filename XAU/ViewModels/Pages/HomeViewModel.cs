...
namespace XAU.ViewModels.Pages
{
    ...
    public partial class HomeViewModel : ObservableObject, INavigationAware
    {
        ...
        #region Update
        // Reads just the CheckForUpdatesEnabled flag from settings.json (default true).
        // Used before the full settings load so the startup update check honours it.
        private bool UpdatesEnabledFromFile()
        {
            try
            {
                ...
                var settings = JsonConvert.DeserializeObject<XAUSettings>(json);
                return settings?.CheckForUpdatesEnabled ?? true;
            }
            catch
            {
                return true;
        ...
        private async Task CheckForToolUpdates()
        {
            ...
            // Respect the user's "check for updates" toggle. This runs before settings are
            // loaded into HomeViewModel.Settings, so read the flag straight from the file
            // (default: enabled).
            if (!UpdatesEnabledFromFile())
                return;

            // The update check points at OUR repo (see GithubRestApi), so an update can
            // only ever be our own build — upstream releases can never overwrite the
            // custom additions. Everything is wrapped so a missing/failed release feed
            // never crashes startup or touches the installed build.
            try
            {
                if (ToolVersion.Contains("DEV"))
                {
                    var jsonResponse = await _gitHubRestAPI.Value.GetDevToolVersionAsync();
                    if (jsonResponse?.LatestBuildVersion == null)
                        return;
                ...
                else
                {
                    ...
                    // No releases published (or feed unavailable): nothing to update to.
                    if (jsonResponse == null || jsonResponse.Count == 0 || jsonResponse[0].tag_name == null)
                        return;
                    // Only newer assets are downloadable; skip if there is no asset.
                    if (jsonResponse[0].assets == null || jsonResponse[0].assets.Count == 0)
                        return;
            ...
            {
                // Update check is best-effort: never let it break startup or the build.
            }
        }
        ...
        private void LoadSettings()
        {
            ...
            Settings.CheckForUpdatesEnabled = settings.CheckForUpdatesEnabled;
        }
