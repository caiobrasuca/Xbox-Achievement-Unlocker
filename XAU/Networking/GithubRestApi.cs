...
public class GithubRestApi
{
    ...
    // Tool-update checks point at OUR repo so an "update" can only ever be our own
    // build — upstream (Fumo) releases can never overwrite the custom additions.
    // (Event DATA sync below still uses Fumo's Events-Data branch: it's just JSON the
    // unlocker needs and can't touch our code.)
    public async Task<VersionResponse?> GetDevToolVersionAsync()
    {
        ...
        var responseString =
            await _httpClient.GetStringAsync("https://raw.githubusercontent.com/caiobrasuca/Xbox-Achievement-Unlocker/Pre-Release/info.json");
