...
public class XboxRestAPI
{
    ...
    // Serializes calls that share the mutable _httpClient.DefaultRequestHeaders, so
    // concurrent operations on the same instance (e.g. the auto-unlocker's parallel
    // verifications + sends + a foreground refresh) can't corrupt each other's headers
    // mid-request. Only the methods that actually run concurrently are wrapped.
    private readonly SemaphoreSlim _gate = new SemaphoreSlim(1, 1);

    private async Task<T> Gated<T>(Func<Task<T>> fn)
    {
    ...
    public Task<GameTitle?> GetGameTitleAsync(string xuid, string titleId) => Gated(async () =>
    {
    ...
    public Task<AchievementsResponse?> GetAchievementsForTitleAsync(string xuid, string titleId) => Gated(async () =>
    {
        if (string.IsNullOrWhiteSpace(xuid) || string.IsNullOrWhiteSpace(titleId))
        {
            // Don't send a request if we don't have the details
            return (AchievementsResponse?)null;
        }
        SetDefaultHeaders();
    ...
    public Task<Xbox360AchievementResponse?> GetAchievementsFor360TitleAsync(string xuid, string titleId) => Gated(async () =>
    {
        if (string.IsNullOrWhiteSpace(xuid) || string.IsNullOrWhiteSpace(titleId))
        {
            // Don't send a request if we don't have the details
            return (Xbox360AchievementResponse?)null;
        }
        SetDefaultHeaders();
    ...
    public Task UnlockTitleBasedAchievementsAsync(string serviceConfigId, string titleId, string xuid, List<string> achievementIds, bool useFakeSignature = false) => Gated(async () =>
    {
        ...
        for (int i = 0; i < achievementIds.Count; i += chunkSize)
        {
            ...
            // Stay under Xbox fine-grained rate limits when sending many chunks.
    ...
    // Sends an NDJSON event batch to OneCollector and returns the HTTP status code
    // (so the caller can retry on 5xx and detect 401/403 token expiry). Builds fresh
    // content each call so it is safe to retry. Returns 0 on a network exception.
